...
export const Communication: React.FC = () => {
    ...
    const submitAnnouncement = async (e: React.FormEvent) => {
        ...
        const recipientsCount = getRecipientCount(annForm.targetAudience);
        let emailStatus = '';

        // Prepare Recipient List for Email
        let recipientList: { name: string; email: string }[] = [];
        if (selectedChannels.includes('EMAIL')) {
            if (annForm.targetAudience === 'ALL') {
                recipientList = students.filter(s => s.active && s.email).map(s => ({ name: s.name, email: s.email }));
            } else {
                // Category ID
                recipientList = students.filter(s => s.active && s.email && s.enrolledCategoryIds.includes(annForm.targetAudience))
        ...
        if (editingAnnId) {
            ...
            if (selectedChannels.includes('EMAIL') && recipientList.length > 0) {
                const res = await sendBulkEmail(recipientList, `Announcement: ${annForm.title}`, annForm.content);
                emailStatus = res.success ? 'Emails Sent Successfully.' : 'Email Sending Failed.';
            }

            alert(`Announcement Updated and re-published to ${recipientsCount} recipients via [${selectedChannels.join(', ')}]. ${emailStatus}`);
        } else {
            // Create New
            addAnnouncement({
                id: `ann_${Date.now()}`,
            ...
            alert(`Announcement Published to ${recipientsCount} recipients via [${selectedChannels.join(', ')}]. ${emailStatus}`);
        }
        resetAnnForm();
    };
